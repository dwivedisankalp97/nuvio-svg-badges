...
```text
https://raw.githubusercontent.com/dwivedisankalp97/nuvio-svg-badges/refs/heads/main/dist/v6/badges.json
```

The pack is generated from `badges.config.json` and `generate-badges.mjs` so
badge names, regex patterns, colors, and SVG assets stay consistent. The v6 URL
...
The v6 SVGs render label text as Roboto Condensed vector paths, so display is not
...
```bash
BADGE_BASE_URL="https://raw.githubusercontent.com/<owner>/<repo>/refs/heads/main/dist/v6" node generate-badges.mjs
```
